const CACHE_NAME = 'agenda-polare-v14.9';
const CACHE_CDN  = 'agenda-polare-cdn-v14.9';
...
self.addEventListener('install', e => {
  e.waitUntil(Promise.all([
    ...
    caches.open(CACHE_CDN).then(c=>Promise.allSettled(CDN_URLS.map(url=>
      fetch(url,{mode:'cors'}).then(r=>{if(r.ok)c.put(url,r);}).catch(()=>{})))),
  ]).then(()=>self.skipWaiting()));
});
self.addEventListener('activate', e => {
  e.waitUntil(caches.keys().then(keys=>Promise.all(
    keys.filter(k=>k!==CACHE_NAME&&k!==CACHE_CDN).map(k=>caches.delete(k))
...
self.addEventListener('fetch', e => {
  ...
  if(url.includes(self.location.origin)){
    e.respondWith(caches.match(e.request).then(c=>c||fetch(e.request).then(r=>{
      if(r.ok){const cl=r.clone();caches.open(CACHE_NAME).then(c=>c.put(e.request,cl));} return r;
    }))); return;
  }
  if(url.includes('unpkg.com')||url.includes('gstatic.com')||url.includes('fonts.google')){
    e.respondWith(fetch(e.request).then(r=>{
      if(r.ok){const cl=r.clone();caches.open(CACHE_CDN).then(c=>c.put(e.request,cl));} return r;
